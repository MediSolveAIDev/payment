...
| 항목 | 결정 | 이유 |
|------|------|------|
| 비운영 환경 전용 | 서버 실행 환경 `settings.environment`(`"dev"/"test"/"prod"`)가 `"prod"`이면 MINUTE 주기 생성을 검증 계층에서 거부. 수정(update)는 차단하지 않음 | 운영 DB에 분 단위 구독이 쌓이는 것 방지 |
...
- `app/models/enums.py` — `BillingCycle`에 `MINUTE = "MINUTE"` 추가
- `app/services/billing_math.py` — `compute_period_end()`에 `MINUTE` 분기 추가 (`timedelta(minutes=cycle_minutes)`)

### Task 2 — Plan 모델 + 마이그레이션
- `app/models/plan.py` — `cycle_minutes: Mapped[Optional[int]]` 컬럼 추가
- `alembic/versions/<rev>_add_minute_billing_cycle.py` — `cycle_minutes` 컬럼 추가 마이그레이션

### Task 3 — 서비스 레이어 검증 + `create_plan`
- `app/services/plans.py` — `_validate_plan_fields()`에 MINUTE 검증 추가 (cycle_minutes 필수·최소 5분·`settings.environment == "prod"` 생성 가드); `create_plan()` 시그니처에 `cycle_minutes` 파라미터 추가, 비운영 가드는 `settings.environment` 기반으로 검증 계층에서 적용
...
- `app/services/renewals.py` — 자동갱신 기간 계산 호출부에 `cycle_minutes=plan.cycle_minutes` 전달

### Task 5 — 외부 API 스키마
- `app/schemas/api.py` — `PlanResponse`에 `cycle_minutes: Optional[int]` 필드 추가 및 `from_model()` 매핑

### Task 6 — 어드민 라우트
- `app/admin/routes/plans.py` — `_form_plan_fields()`에 `billing_cycle == MINUTE`일 때 `cycle_minutes` 파싱 추가; `plans_create` / `service_plan_create` 호출부에 `cycle_minutes` 전달
...
- 운영 서버(`settings.environment == "prod"`)에서는 MINUTE 요금제 생성이 검증 계층에서 거부된다.
