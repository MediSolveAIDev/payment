...
**취소 카드** — `kind == ONE_OFF` AND `status == DONE`일 때만 표시 (구독 결제·이미 취소된 결제는 카드 자체가 없음):
- `service.cancellation_enabled == True` → 결제 취소 버튼 + `confirm()` 팝업
- `service.cancellation_enabled == False` → "취소 불가(서비스 정책)" 뱃지만 표시

**취소·환불 내역 카드** — `status == CANCELED`일 때 표시. 다음을 표로 보여준다:
- 총 결제금액(`amount`) · 환불액(`canceled_amount`, 빨강) · 취소 수수료(`cancel_fee`, 서비스 보유) · 취소 시각(`canceled_at`)
...
결제 상세 화면의 "취소" 카드에서 "결제 취소" 버튼을 클릭한다.

**취소 가능 조건** (조건 하나라도 맞지 않으면 버튼 자체가 없거나 서버에서 오류 반환):

1. `payment.kind == ONE_OFF` — 구독 결제는 취소 불가
2. `payment.status == DONE` — PENDING·FAILED·CANCELED 상태는 취소 불가
3. `service.cancellation_enabled == True` — 서비스 정책에서 취소가 켜져 있어야 함

**취소 수수료 미리보기**: 취소 카드에는 버튼 위에 "지금 취소하면 수수료 **N원**(수수료율 X%)이 차감되고 **M원**이 환불됩니다." 안내가 표시된다. 라우트 `payment_detail`이 `compute_cancel_fee(payment.amount, service.cancellation_fee_percent)`로 예상 수수료/환불액을 계산해 템플릿에 전달한다(`cancel_fee`/`cancel_refund`).

**버튼 클릭 시 흐름:**

1. 브라우저 `confirm()` 팝업: "이 결제를 취소(환불)하시겠습니까? 수수료 {N}원 차감 후 {M}원 환불" — **수수료/환불액을 금액(원)으로** 안내.
2. 확인 → `POST /admin/payments/{id}/cancel` (CSRF 토큰 hidden 포함).
3. 서버: CSRF 검증 → 결제 조회 → 스코프 확인 → `cancel_one_off_payment()` 호출.
4. 성공 → 303 리다이렉트 → 결제 상세 페이지 + 완료 모달("결제가 취소되었습니다").
5. 실패(토스 오류 등) → 에러 토스트.
...
- 외부 API 응답·취소 미리보기·실제 취소가 모두 `compute_cancel_fee()` 한 함수를 공유한다.
- 수수료율 0%이면 전액 환불(토스 cancelAmount 생략 = 전액취소).
- 수수료율 > 0이면 부분취소(토스 cancelAmount = 환불액).
- `refund == 0`이면 토스 취소 API를 호출하지 않는다.

**감사 로그 actor 분기** (`payments.py:225–232`):

- 어드민에서 취소: `actor_type="USER"`, `actor_user_id=ctx.user.id` (실제 관리자 계정 기록)
- 외부 API에서 취소: `actor_type="SERVICE"`, `actor_service_id=service.id`
