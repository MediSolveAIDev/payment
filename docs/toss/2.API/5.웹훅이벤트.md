...
keyword: 가상계좌 입금 알림, 웹훅, 콜백, 웹훅 이벤트, 알림, Webhook, PAYMENT_STATUS_CHANGED, DEPOSIT_CALLBACK, PAYOUT_STATUS_CHANGED, METHOD_UPDATED, CUSTOMER_STATUS_CHANGED, 입금 알림, 상태 변경, 입금 확인, 가상계좌 테스트, 입금 테스트, 입금 확인, 입금 기한 만료, 결제 만료, 결제 취소, 결제수단 변경, 회원 탈퇴
---
...
> **경고**: [자동결제](/guides/v2/billing/integration)는 결제 요청에 이어서 승인이 되기 때문에 결제가 완료됐을 때 웹훅을 전송하지 않습니다.
...
> **경고**: 만약 [`PAYMENT_STATUS_CHANGED`](#payment_status_changed), [`DEPOSIT_CALLBACK`](#deposit_callback) 이벤트를 모두 등록했다면 가상계좌 상태가 변경될 때 웹훅이 두 번 전송됩니다.
...
링크페이 주문의 결제 상태가 변경되면 웹훅이 전송됩니다. 결제가 승인(`DONE`)되거나 취소(`CANCELED`), 부분 취소(`PARTIAL_CANCELED`)될 때 웹훅을 받을 수 있어요.

```mermaid
---
title: 링크페이 주문 결제 상태(status) 흐름
---
%%{
  init: {
    'themeVariables': {
...
flowchart TD
    ...
    id1([결제 링크 접속]):::plain -->id2
    id2([결제 진행]):::noWebhook -->|결제 완료| id3([DONE]):::yesWebhook
    id3 -->|결제 취소| id4([CANCELED]):::yesWebhook
    id3 -->|결제 부분 취소| id5([PARTIAL_CANCELED]):::yesWebhook
...
상태가 변경된 [Order 객체](/reference/linkpay#order-객체)입니다. `payment` 필드에 [Payment 객체](/reference#payment-객체)가 포함되어 있어요.

## 지급대행

### payout.changed

지급대행 요청 상태가 `COMPLETED`, `FAILED`로 바뀌면 웹훅이 전송됩니다. `REJECTED` 상태일 때는 `FAILED`로 웹훅이 발송됩니다. `DELETED` 상태에서는 웹훅이 발송되지 않습니다.
