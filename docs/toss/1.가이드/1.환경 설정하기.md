...
- 테스트 환경에서 발급된 계좌로 직접 입금할 수 없지만 [테스트 결제내역 메뉴](https://developers.tosspayments.com/my/payment-logs)에서
  입금처리를 할 수 있습니다.
- 테스트 환경에서는 가상계좌 안내 문자가 발송되지 않아요.
...
- 연동 테스트 상점에서 거래대사 API를 호출하면 최근 3일 이내의 거래만 조회할 수 있어요.

지급대행

- 테스트 환경에서 [잔액조회 API](/reference#잔액-조회)를 호출하면 응답으로 `availableAmount`만 돌아옵니다.
- 테스트 환경에서는 유효한 사업자번호로 법인사업자(`CORPORATE`) [셀러만 등록](/reference/additional#셀러-등록)할 수 있어요. 셀러의 상태가 `KYC_REQUIRED`로 바뀔 수는 있지만 테스트 KYC 수행이 불가능하기 때문에 `APPROVED` 상태로 바뀔 수는 없습니다.
- [지급대행 요청 API](/reference/additional#지급대행-요청)를 라이브 환경에서 테스트하면 수수료가 부과됩니다.

계좌인증

- 테스트 환경에서는 계좌 인증 시 Mock 데이터가 응답됩니다.
- 일치 확인 검증은 항상 성공으로 처리되며, 예금주명은 `홍길동`으로 고정되어 응답됩니다.
