...
title: 토스페이먼츠 SDK 에러 코드
description: 토스페이먼츠 SDK를 사용할 때 발생할 수 있는 모든 에러를 살펴보세요.
keyword: 에러, SDK, 결제창, 오류, error, failure, 에러 객체
...
| `INVALID_METADATA`             | 올바르지 않은 metadata 형식입니다.                                                                                               |
| `NOT_SUPPORTED_WIDGET_KEY`     | API 개별 연동 키의 클라이언트 키로 SDK를 연동해주세요. 결제위젯 연동 키는 지원하지 않습니다.                                     |
| `NO_ACTIVE_PAYMENT_REQUEST`    | 진행 중인 결제 요청이 없어요.                                                                                                    |
| `PAYMENT_REQUEST_ABORTED`      | 결제 요청이 중단됐어요.                                                                                                          |

## failUrl로 전달되는 에러

결제위젯, 결제창의 `requestPayment()` 메서드로 결제 요청에 실패했을 때 `failUrl`로 전달되는 에러 목록입니다.
...
브랜드페이의 `requestPayment()` 메서드로 결제 요청에 실패했을 때 `failUrl`로 전달되는 에러 목록입니다.
