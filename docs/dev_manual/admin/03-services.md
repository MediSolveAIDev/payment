...
**취소 정책 카드 + 서비스 알림 카드**(나란히 배치): 왼쪽은 일반결제 취소 허용 체크박스 + 수수료율(%), 오른쪽은 **서비스 알림 URL** 입력(요청 016).
- 알림 URL을 등록하면 구독·결제·카드·요금제 이벤트를 그 URL로 POST한다(비우면 끔). 저장 `POST /admin/services/{id}/notification-url`(감사 `service.notification_url_updated`).
- **‘테스트 알림 전송’ 버튼**: 저장된 URL로 샘플 알림(`notification.test`)을 동기 전송하고 수신 응답을 토스트로 표시(`POST /admin/services/{id}/notification-test`). URL 미저장이면 비활성.
- ‘상세’ 버튼 → 이벤트 설명 모달(`services/_notify_help_modal.html`). 발송 이벤트·payload·서명은 `17-service-notifications` 참조.

**하단 탭 4개** (htmx 부분 갱신, 각 partial)

| 탭 | `id` | 부분 템플릿 |
|----|------|------------|
| 요금제 | `list-svc-plans` | `services/_plans_table.html` |
| 구독 | `list-svc-subs` | `services/_subs_table.html` |
| 등록 카드 | `list-svc-cards` | `services/_cards_table.html` |
...
#### 등록 카드 탭 컬럼 (`_cards_table.html`)

사용자(external_user_id) / 카드번호(마스킹) / 발급사코드 / **상태** / customerKey / 빌링키 해시(앞 12자) / 등록일 / 변경일 / **토글 버튼**

- 이 서비스에 결제수단(빌링키)을 등록한 **사용자별 1행**(`cards` 테이블, `(service_id, external_user_id)`당 1건)을 보여준다(`services.py` `_cards_tab()`).
- **상태** 컬럼은 활성/비활성 뱃지, 마지막 컬럼의 **활성화/비활성화 버튼**(htmx `POST /admin/cards/{id}/toggle` → `list-svc-cards` 갱신)으로 토글한다. **비활성 카드는 모든 결제가 차단**된다(`16-card-vault` 10-1 참조).
- **행 클릭 → 카드 상세**(`/admin/cards/{card_id}`): 카드 정보 전체 + 토글 + **이 카드로 결제한 내역**(구독+일반).
- 검색(`q`: external_user_id) 지원, 정렬 가능 컬럼: 사용자 / 등록일 / 변경일. 페이징 파라미터는 `kpage`(10건/페이지)로 다른 탭과 분리된다.
- 카드번호·발급사코드는 토스 응답 `card_info`(JSONB)에서 표시하며, **빌링키 암호문은 노출하지 않는다**(해시 일부만 표시). 카드 등록/삭제는 외부 API(`/api/v1/cards`) 또는 구독 상세에서 처리한다.

#### 일반결제 탭 컬럼 (`_oneoff_table.html`)

승인시각 / 사용자 / 주문번호 / 금액 / **환불 / 수수료** / 상태(한글)

- **행 클릭 → 결제 상세**(`/admin/payments/{id}`). 주문번호 셀은 링크색으로 강조된다.
...
- **카드 이벤트**: `target_type='card'` 중 `detail.service_id`가 이 서비스 — 카드 등록/교체/삭제/활성화/비활성화. 상세에 사용자·마스킹 카드번호·발급사가 표시된다.
...
| 허용 IP | 옥텟 입력 UI (숫자만, 줄당 1개) | 선택 | IPv4 전용, IPv6·CIDR 불가. **비우면 IP 제한 없음(모든 IP 허용, HMAC로만 보호)** |
...
- IPv4 전용. **빈 목록 허용**(= IP 제한 없음, 모든 IP 허용). 형식 위반 시 오류 → `?error=` 리다이렉트
...
| `registry.update_allowed_ips` | `app/services/registry.py:193` | IP 목록 전체 교체 (빈 목록 허용 = IP 제한 없음) |
...
### IP 관련
- 허용 IP는 **선택**이다. **빈 목록 = IP 제한 없음(모든 IP 허용)** — 이 경우 API는 HMAC 서명으로만 보호된다 (`api/deps.py`).
- 값이 있으면 IPv4 형식만 허용(IPv6·CIDR 불가) — `_validate_ips()` (`registry.py`).
