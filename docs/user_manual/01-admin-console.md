...
> 함께 보기: [서비스 관리](02-admin-services.md) · [카드 관리](03-admin-card.md) · [구독 관리](04-admin-subscription.md)
...
> 참고: 로컬 개발 환경(`environment == "dev"`)에서는 편의를 위해 로그인 폼에 개발용 이메일·비밀번호가 미리 채워질 수 있습니다. 스테이징(stg)·운영(prod) 등 외부에 노출되는 환경에서는 자동입력이 동작하지 않으므로, 실제 콘솔에서는 항상 직접 입력해야 합니다.
...
![로그인 직후 대시보드와 왼쪽 사이드바 메뉴](assets/img/dashboard.png)
...
| **구독** | 구독 목록을 검색·필터링하고, 구독 상세에서 강제취소·만료일 연장·재결제를 합니다. → [구독 관리](04-admin-subscription.md) |
| **결제** | 개별 결제 내역을 확인합니다. |
...
| 메뉴 | 하는 일 |
|------|--------|
| **서비스** | 구독·결제를 사용할 서비스를 등록하고, 서비스 키를 발급합니다. 서비스 상세에서 **등록 카드**(카드 보관함)도 봅니다. → [카드 관리](03-admin-card.md) |
...
- **매뉴얼 열기**: 화면 **상단 오른쪽의 책 아이콘**을 누르면 이 매뉴얼(사용·개발 매뉴얼)이 **새 창(탭)** 으로 열립니다.
- **다크/라이트 모드**: 화면 **상단 오른쪽의 달/해 아이콘 버튼**을 누르면 누를 때마다 어두운 화면(다크)과 밝은 화면(라이트)이 즉시 전환됩니다. 선택은 이 브라우저에 저장되어 다음에 접속해도 유지되며, 처음에는 사용 중인 OS 설정(다크 모드 여부)을 따릅니다.

> 팁: 표에서 무언가를 자세히 보고 싶으면, 버튼을 찾기 전에 일단 **그 줄을 클릭**해 보세요. 대부분 상세 화면으로 바로 이동합니다.

---

> 함께 보기: 카드(결제수단) 관리는 [카드 관리](03-admin-card.md), 구독의 상태와 운영 작업은 [구독 관리](04-admin-subscription.md)에서 이어집니다.
