...
5. **빌링키 복호화 + 토스 청구** — 카드의 `billing_key_encrypted`를 `cipher.decrypt`로 풀어 `resolve_charge(...)`로 청구한다. 토스에는 클라이언트 `order_id`가 아닌 전역 고유 `toss_order_id`를 전달한다(서비스 간 멱등키 충돌 방지) (`app/services/payments.py:146`~`160`).
   - **타임아웃 후 재확정** — `resolve_charge`는 `toss.charge`가 `TossTimeoutError`(또는 2xx인데 본문 해석 불가)를 던지면 곧바로 실패로 보지 않고 `toss.get_payment_by_order_id(toss_order_id)`로 **재조회**한다. 재조회 결과가 `status=="DONE"`이면 그 결과를 성공으로 확정하고, 그 외(미발견·비DONE·재조회 자체 실패)면 원래의 `TossTimeoutError`를 다시 던진다 (`app/services/payment_utils.py:46`~`57`). 즉 "토스가 실제로는 승인했는데 응답만 못 받은" 경우를 한 번 더 건져낸다.
6. **결과 분기**
   - `TossTimeoutError`(재조회 후에도 결과 불명): **절대 FAILED로 만들지 않고** PENDING 유지, 감사(`payment.one_off_unresolved`), HTTP 503으로 `PaymentFailedError` (`app/services/payments.py:161`~`172`).
...
## 16.4 흐름 3 — 어드민 전액/부분 취소(수수료 없음·누적)
...
5. **알림은 best-effort** — `notifier.send(...)` 실패가 결제/취소를 깨면 안 된다. 알림 누락이 의심되면 [17. 서비스 알림](17-feature-notifications.md)을 본다.
