...
> 함께 보기: [서비스 API](13-service-api.md)
...
| uv                      | 의존성·실행 관리(`uv sync`/`uv run`). **로컬 개발**에서 앱을 직접 실행할 때 필요(운영은 컨테이너가 담당) |
...
| ------------------------- | ----------------------------------------------------------------- | --------------------------------------------- | ------- |
| **호스트 nginx**          | VM에 `apt`로 설치. TLS 종단·리버스 프록시, certbot 발급·자동 갱신 | **80 / 443** (유일하게 인터넷 공개)           | 12.5.4  |
| **docker A — PostgreSQL** | `db_server/run.sh`로 `postgres:16` 컨테이너(`payment-postgres`)   | 5432 (VM 내부/사설망만 — 인터넷 금지)         | 12.5.2  |
...
> 대안: DB를 자체 docker 대신 **관리형(Azure Database for PostgreSQL)**으로 쓸 수도 있다. 그 경우 12.5.2(docker A)를 건너뛰고 `.env.prod`의 `DATABASE_URL`만 관리형 엔드포인트로 두면 된다(`?ssl=require` 필요할 수 있음).
...
> 참고: `docker compose up -d`(인자 없음)는 루트 `docker-compose.yml`(프로젝트명 `payment-dev`)을 띄운다 — **Redis만** 올리고 포트를 루프백(`127.0.0.1:6380:6379`)에만 바인딩한다. 앱까지 컨테이너로 띄우려면 `docker compose -f docker-compose.dev.yml up -d --build`(또는 `./dev.sh`)를 쓰며, 이때 app은 `host.docker.internal:5432`로 호스트의 `payment-postgres`에 접속한다.

> 참고: 최초 관리자 계정은 `uv run python -m app.cli create-admin`으로 만든다(`pyproject.toml`의 콘솔 스크립트를 통해 `uv run payment-system create-admin`도 동일). SYSTEM_ADMIN 권한으로 생성되며, 같은 이메일이 이미 있으면 거부된다.
...
전체 변수는 `app/core/config.py`의 `Settings`에 정의된다. `.env`(공통) → `.env.<APP_ENV>`(환경별, 뒤가 우선) 순으로 로드되며, `APP_ENV` 미지정 시 `dev`. 운영 일부 값은 compose `environment`가 다시 덮어쓴다(아래 마지막 참고). 선언되지 않은 변수는 `extra="ignore"`로 무시된다.

### 12.3.1 핵심·인프라

| 변수                               | 설명                                                                             | 기본·예시                                                            |
| ---------------------------------- | -------------------------------------------------------------------------------- | -------------------------------------------------------------------- |
| `ENVIRONMENT` / `APP_ENV`          | 실행 환경(`dev`/`test`/`stg`/`prod`). `prod`면 HSTS 부착·docs 기본 비공개·세션 쿠키 보안 강화. `APP_ENV`가 `ENVIRONMENT`보다 우선 | `dev` / `prod`                                                       |
...
| `TOSS_WEBHOOK_ALLOWED_IPS`                                   | 토스 발신 허용 IP(JSON 배열). `.env`에 두며 기본은 코드 내장 목록 | `["13.124.18.147", ...]`           |
...
| `DEV_LOGIN_EMAIL` / `DEV_LOGIN_PASSWORD`                     | 로그인 폼 기본값 자동 채움. **`dev`에서만** 노출(stg·prod 무시)  | (비움)                             |
...
| `EXPORT_MAX_ROWS`                                     | 엑셀 내려받기 1건 최대 행 수(워커 OOM 방지)                     | `100000`      |
...
> 중요: `ENCRYPTION_KEY`·DB 비밀번호·`SWAGGER_PW`·`GMAIL_PW` 등 비밀값은 Git에 커밋하지 않는다. `.env.example`만 추적한다.
> `TOSS_SECRET_KEY`는 2026-06-23부로 제거됨 — `.env.example`에서도 삭제 대상.

> 참고: 운영에서 `APP_ENV`·`REDIS_URL`·`TRUST_PROXY`·`TRUST_PROXY_HOPS`는 compose `environment`가 고정 주입한다(`.env.prod`에 적어도 compose 값이 우선, `docker-compose.prod.yml`). `dev`/`stg` compose도 동일하게 `APP_ENV`·`REDIS_URL`을 고정하고, dev/stg는 `DATABASE_URL`을 `host.docker.internal`로 덮어쓴다.

> 참고: 다중 인스턴스로 늘릴 때 마이그레이션 중복 적용을 막는 `RUN_MIGRATIONS`(엔트리포인트, `0`이면 건너뜀)는 `Settings`가 아닌 컨테이너 환경변수다. 자세한 건 12.5.3·12.6 참조.
...
- **빈 `payment` DB만 있으면 된다** — 테이블(스키마)은 app이 자동 생성한다(12.5.3의 마이그레이션).
