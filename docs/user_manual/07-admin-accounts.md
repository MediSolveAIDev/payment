...
> 함께 보기: [전체 설정](08-admin-settings.md) · [감사 로그](09-admin-audit.md)
...
| 도구 | 설명 |
|---|---|
| 검색창 | 이메일 일부를 입력하면 해당 계정만 추려집니다(부분 일치). |
| 역할 필터 | 시스템 관리자 / 서비스 담당자로 좁혀 봅니다. |
| 상태 필터 | 활성(ACTIVE) / 설정 대기(PENDING) / 잠김(LOCKED) / 비활성(DISABLED)으로 좁혀 봅니다. |
| 정렬 | 이메일 · 역할 · 상태 · 생성일 기준으로 정렬할 수 있습니다(기본은 생성일 최신순). |
| 엑셀 다운로드 | 현재 검색·필터 조건 그대로 목록을 엑셀(.xlsx) 파일로 내려받습니다. 내려받는 항목은 **이메일 · 역할 · 주 서비스 · 상태**이며, 페이지에 보이는 분량이 아니라 조건에 맞는 **전체**가 담깁니다. |
...
| 잠김 (LOCKED) | 로그인 비밀번호를 연속으로 여러 번 틀려 일시 잠김 (정해진 시간이 지나면 자동 해제) |
| 비활성 (DISABLED) | 관리자가 일부러 막아 둔 상태 → 로그인 불가 |
| 삭제됨 (DELETED) | 삭제 처리된 계정 → 목록에 보이지 않음(완전히 지우지 않고 숨김) |

> 참고: 비밀번호를 몇 번 틀리면 잠기는지(기본 5회), 얼마 후 자동 해제되는지(기본 15분)는 [전체 설정](08-admin-settings.md)에서 바꿀 수 있습니다.
...
계정이 만들어지면 **설정 대기(PENDING)** 상태가 되고, 입력한 이메일로 **비밀번호 설정 메일**(「비밀번호 설정하기」 버튼이 있는 HTML 메일, 비밀번호 재설정 메일과 동일한 디자인)이 자동 발송됩니다. 받는 사람이 메일의 버튼/링크에서 비밀번호를 직접 정하면 활성(ACTIVE) 상태가 되어 로그인할 수 있습니다.

> 참고: 서비스 담당자에게 여러 서비스를 한 번에 배정하면, 그중 **첫 번째 서비스가 "주 서비스"**(목록에 표시되는 대표 서비스)가 되고 나머지는 추가 담당으로 들어갑니다. 같은 서비스를 두 번 골라도 한 번만 반영됩니다.

> 중요: 비밀번호 설정 링크는 **발송 후 48시간** 동안만 유효합니다(기본값, [전체 설정](08-admin-settings.md)에서 조정 가능). 만료되면 계정 상세에서 다시 발송하면 됩니다.
...
> 참고: 전화번호 칸을 비운 채 저장하면 등록된 전화번호가 지워집니다. 다른 계정이 이미 쓰는 이메일로는 바꿀 수 없습니다(중복 안내가 표시됩니다).

---

## 7.5 담당 서비스 배정 · 해제

서비스 담당자(SERVICE_MANAGER) 계정 상세 화면 오른쪽에는 **담당 서비스** 카드가 있습니다. 드롭다운에는 **아직 담당하지 않은 서비스만** 나타나므로, 같은 서비스를 두 번 배정할 일은 없습니다.
...
> 참고: 담당 서비스가 하나도 없던 담당자에게 처음 배정한 서비스는 자동으로 **주 서비스**가 됩니다.
...
> 참고: 주 서비스를 해제하면, 남아 있는 담당 서비스 중 하나가 자동으로 새 주 서비스로 올라섭니다. 남은 서비스가 없으면 담당 서비스가 비게 됩니다.

> 중요: 해당 서비스의 **대표 담당자**(알림 수신처)로 지정된 계정은 그 서비스에서 해제할 수 없습니다. "대표 담당자는 해제할 수 없습니다" 안내가 뜨면, 먼저 서비스 화면에서 다른 계정을 대표로 지정한 뒤 해제하세요.
...
## 7.8 비밀번호 설정 메일 다시 보내기

비밀번호를 잊었거나, 설정 링크가 만료됐거나, 처음 메일이 도착하지 않았을 때 사용합니다.

<ol class="steps">
<li>계정 상세에서 <b>비밀번호 재설정 메일</b> 버튼을 누릅니다.</li>
<li>메일은 <b>발송 대기열(메모리 큐)에 등록</b>되고 "비밀번호 재설정 메일 발송을 요청했습니다(대기열)" 토스트가 바로 표시됩니다. 실제 전송은 서버가 대기열에서 <b>순서대로</b> 처리합니다(전송 결과는 감사 로그에 기록 — [감사 로그](09-admin-audit.md) 참고).</li>
<li>입력된 이메일로 <b>비밀번호 재설정 버튼이 있는 안내 메일</b>(브랜딩·CTA 버튼 포함 HTML 메일, HTML 미지원 클라이언트는 평문으로 표시)이 발송됩니다.</li>
<li>받는 사람이 메일의 <b>「비밀번호 재설정하기」</b> 버튼(또는 본문 링크)에서 새 비밀번호를 정하면 다시 로그인할 수 있습니다.</li>
</ol>

> 참고: 보안을 위해 재설정 메일을 보내면 해당 계정의 기존 로그인 세션은 즉시 모두 종료됩니다. 새 링크 유효기간은 역시 **48시간**입니다.

> 팁: 비밀번호를 연속으로 여러 번(기본 5회) 틀려 **잠김(LOCKED)** 상태가 된 계정도, 이 재설정 메일로 새 비밀번호를 설정하면 잠금이 바로 풀립니다. (또는 정해진 시간이 지나면 자동 해제됩니다 — 기본 15분.)
...
- [감사 로그 (누가 무엇을 했나)](09-admin-audit.md)
