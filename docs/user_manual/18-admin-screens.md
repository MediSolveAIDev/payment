...
하는 일: 로그인은 IP당 분당 시도 제한(`_login_rate_limited`, `app/admin/routes/auth.py:31`)을 먼저 검사한 뒤 `auth_service.login`으로 인증하고, 성공 시 `admin_session` 쿠키(HttpOnly, SameSite=Lax, prod에서만 secure)를 발급하고 `/admin`으로 보냅니다. 로컬 개발(`environment == "dev"`)에서만 폼에 `dev_login_email`/`dev_login_password`를 미리 채우며, 스테이징(stg)·운영(prod)에서는 채우지 않습니다. 비밀번호 설정/재설정은 메일로 받은 `token`을 hidden으로 전달받아 처리하며, 확인 불일치는 폼 단에서 즉시 오류로 막습니다.
...
> 라우트 파일: `app/admin/routes/guide.py` · 템플릿: `guide.html`
> 문서 소스: `docs/admin_manual/` 디렉터리의 마크다운(`.md`)을 런타임에 HTML로 변환해 렌더

어드민 콘솔 안에서 관리자용 매뉴얼을 읽는 화면입니다. 라우터는 `prefix="/guide"`로 등록됩니다(`app/admin/__init__.py:137`).

| 경로 | 라우트 함수 | 템플릿 | 권한 |
|------|-------------|--------|------|
| `GET /admin/guide` | `guide_index` `app/admin/routes/guide.py:42` | `guide.html`(기본 `dashboard` 가이드) | `require_any` |
| `GET /admin/guide/{guide_id}` | `guide_page` `app/admin/routes/guide.py:23` | `guide.html` | `require_any` |

하는 일: `GUIDES` 목록(`app/admin/routes/guide.py:13`)에 정의된 `guide_id`→파일 매핑으로 `docs/admin_manual/` 아래 마크다운을 찾아 `markdown`(extensions: `extra`/`toc`/`sane_lists`/`attr_list`)으로 변환해 좌측 가이드 메뉴(`guides`)와 함께 렌더합니다. `guide_id`가 매핑에 없거나 파일이 없으면 404입니다. `GET /admin/guide`(index)는 기본값으로 `dashboard` 가이드를 그대로 위임 렌더합니다.

현재 제공 가이드(`guide_id` → 문서):

| `guide_id` | 제목 | 문서 파일 |
|------------|------|-----------|
| `dashboard` | 대시보드 | `docs/admin_manual/01-dashboard.md` |
| `subscriptions` | 구독 관리 | `docs/admin_manual/02-subscriptions.md` |
| `payments` | 결제 관리 | `docs/admin_manual/03-payments.md` |
| `plans` | 요금제 관리 | `docs/admin_manual/04-plans.md` |
| `services` | 서비스 관리 | `docs/admin_manual/05-services.md` |
| `settlement` | 정산 관리 | `docs/admin_manual/06-settlement.md` |
| `audit` | 감사 및 계정 | `docs/admin_manual/07-audit-and-users.md` |

> 참고: 이 화면이 읽는 `docs/admin_manual/`은 **관리자(사용자)용** 매뉴얼이고, 지금 보고 있는 `docs/user_manual/`(개발자용)과는 별개 디렉터리입니다. SERVICE_MANAGER도 접근할 수 있으나, 가이드 내용은 권한별로 분기하지 않습니다.
