...
> 함께 보기: [서비스 API](13-service-api.md), [일반결제·취소·정산](16-feature-payment.md)
...
## 17.4 발송 흐름(HttpServiceNotifier.send) — best-effort
...
## 17.5 알림 URL 등록 + 어드민 테스트 버튼

### URL 저장

`app/admin/routes/services.py:441`의 `services_notification_url`. CSRF 검증 후 폼 `notification_url`을 받는다.

- 값이 있으면 `http://` / `https://`로 시작해야 한다. 아니면 `?error=`로 안내 후 리다이렉트 (`services.py:453`~`456`).
- 빈 값이면 `service.notification_url = None`(알림 끔)으로 저장 (`services.py:461`).
- 감사 로그 `service.notification_url_updated`(old/new URL 기록) (`services.py:462`~`465`).

### 테스트 발송(동기)

`app/admin/routes/services.py:470`의 `services_notification_test`. 일반 `send`와 달리 **동기**로 보내 수신 측 응답을 운영자에게 즉시 토스트로 보여준다.

```python
# app/admin/routes/services.py:484
...
2. **수신 처리** — `notify_receive_view`(`sample_service/shop/views.py:668`)가 본문 JSON + `X-Signature`/`X-Timestamp`/`X-Nonce`를 검증한 뒤 `EVENT`/`STATUS` 등을 꺼내 저장한다(`views.py:682`~`686`).
...
- 알림은 **한 번만** POST하며 **재시도·재전송이 없다**(17.4). 수신 서버가 다운·타임아웃·5xx면 그 이벤트는 **영구 유실**된다.
- 따라서 **중요한 상태는 알림에만 의존하지 말 것.** 구독·결제의 최종 상태는 조회 API(`GET /api/v1/subscriptions/{external_user_id}` · `GET /api/v1/payments/{external_user_id}`, [서비스 API](13-service-api.md))로 **주기적으로 재확인**하거나, 사용자 접근 시점에 `access_allowed`를 다시 조회해 보정한다.
...
- 4xx/5xx를 반환해도 발송 측은 **로그만 남기고 흡수**한다(별도 통지·재시도 없음). 단, 어드민 '테스트 알림'은 동기 발송이라 이 응답 코드가 운영자 토스트로 보인다(17.5).
...
| URL 형식 오류(어드민 저장) | `?error=`로 안내, 저장 안 함(`services.py:453`) |
