...
> 함께 보기: [카드 기능 코드 흐름](12-feature-card.md) · [구독 기능 코드 흐름](13-feature-subscription.md) · [결제 기능 코드 흐름](14-feature-payment.md) · [서비스 알림](15-feature-notifications.md)
...
| `GET /api/v1/cards/{external_user_id}` | HMAC | 등록 카드 마스킹 정보 조회(없으면 404) | `app/api/v1/cards.py:80` |
| `DELETE /api/v1/cards/{external_user_id}` | HMAC | 카드·빌링키 삭제(204) | `app/api/v1/cards.py:111` |
...
| 필드 | 타입 | 제약 | 설명 |
|------|------|------|------|
| `external_user_id` | string | 1–255자 | 외부 서비스 측 사용자 식별자 |
...
| 필드 | 타입 | 설명 |
|------|------|------|
| `external_user_id` | string | 외부 서비스 측 사용자 식별자 |
...
| 필드 | 타입 | 제약 | 설명 |
|------|------|------|------|
| `external_user_id` | string | 1–255자 | 외부 서비스 측 사용자 식별자(서비스+사용자 당 구독 1개 기준) |
...
- **best-effort(fire-and-forget)**: 알림 전송은 백그라운드 단발 POST(타임아웃 5초)로 처리되며, **실패해도 재시도하지 않습니다**(결제·구독 본 처리에는 영향 없음, 로그만 남김). 수신 측 전달 보장·멱등 처리 규약은 [서비스 알림](15-feature-notifications.md)을 반드시 참고하세요.
...
11.2.3의 `call()` 헬퍼가 있다고 가정하고 **카드 등록 → 구독 생성 → 알림 수신**까지 잇는 최소 흐름입니다. `auth_key`는 11.3.1처럼 **클라이언트 토스 결제창에서 먼저 받아 둔** 값입니다.
...
수신 측(서비스 서버)은 11.6.3의 `verify_notification`으로 서명을 검증한 뒤 자기 DB를 갱신합니다. 단건 결제·취소·구독 취소/재개·내역 조회는 같은 `call()`로 **경로만 바꿔** 호출하면 됩니다(11.4·11.5 표 참고).

> 팁: 동작하는 전체 예제는 `sample_service/`에 있습니다 — 카드 등록 화면, `POST /notify`(서명 검증 후 저장), `/notifications`(받은 알림 목록)까지 한 흐름으로 따라갈 수 있습니다.

---

> 함께 보기: 카드 흐름 → [카드 기능 코드 흐름](12-feature-card.md) · 구독 흐름 → [구독 기능 코드 흐름](13-feature-subscription.md) · 결제 흐름 → [결제 기능 코드 흐름](14-feature-payment.md) · 알림 → [서비스 알림](15-feature-notifications.md)
